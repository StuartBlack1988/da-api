...
class InitialSetup {
        ...
        try {
            ...
            $db->exec("
                CREATE TABLE IF NOT EXISTS `PatientDetails` (
                    ...
                    `communicationPracticeUserId` INT NULL,
                    `featurePracticeBilling` BOOLEAN DEFAULT FALSE,
                    `featurePracticeBookings` BOOLEAN DEFAULT FALSE,
                    `featureMealPlanTemplates` BOOLEAN DEFAULT FALSE,
                    `featureInvoiceTemplates` BOOLEAN DEFAULT FALSE,
                    `createdDate` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    `modifiedDate` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    FOREIGN KEY (`communicationPracticeUserId`) REFERENCES `PracticeUser`(`practiceUserId`),
                    INDEX `idx_patient_idnumber` (`idNumber`),
                    INDEX `idx_patient_name_surname` (`name`, `surname`),
                    ...
                    INDEX `idx_patient_communication` (`communicationPracticeUserId`),
                    INDEX `idx_patient_features` (`featurePracticeBilling`, `featurePracticeBookings`, `featureMealPlanTemplates`, `featureInvoiceTemplates`),
                    INDEX `idx_patient_billing` (`featurePracticeBilling`),
                    INDEX `idx_patient_bookings` (`featurePracticeBookings`),
                    INDEX `idx_patient_mealplans` (`featureMealPlanTemplates`),
                    INDEX `idx_patient_invoices` (`featureInvoiceTemplates`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            ");

            // Add foreign key to PracticeUser after PatientDetails is created
            $db->exec("
                ALTER TABLE `PracticeUser`
                ADD FOREIGN KEY (`patientDetailsId`) REFERENCES `PatientDetails`(`patientDetailsId`)
            ");
            ...
            $db->exec("
                CREATE TABLE IF NOT EXISTS `Dependent` (
                    ...
                    `createdDate` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    `modifiedDate` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    FOREIGN KEY (`patientDetailsId`) REFERENCES `PatientDetails`(`patientDetailsId`) ON DELETE CASCADE,
                    INDEX `idx_dependent_patient` (`patientDetailsId`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            ");
            ...
            $db->exec("
                ...
                ('practice-manager', 'Practice manager with administrative access'),
                ('receptionist', 'Receptionist with limited access'),
                ('dietitian', 'Dietitian with professional access'),
    ...
    public function down($db) {
        ...
        $db->exec("DROP TABLE IF EXISTS `Dependent`");
